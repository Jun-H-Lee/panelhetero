	        ...
	        if (`acov_order' == 0){
			    if("`ci'" == "on"){
			        graph twoway rarea acov_UCI acov_LCI acov_grid, color(gs14) || line acov_ecdf acov_grid, ytitle("Cumulative Distribution") ///
	                              xtitle("Variance") title("Empirical CDF Estimation for Variance") legend(cols(2) order(2 "point estimate" 1 "95% C.I." ))
			    ...
			    if("`ci'" == "on"){
			        graph twoway rarea acov_UCI acov_LCI acov_grid, color(gs14) || line acov_ecdf acov_grid, ytitle("Cumulative Distribution") ///
	                              xtitle("Autocovariance") title("Empirical CDF Estimation for Autocovariance of order `acov_order'") legend(cols(2) order(2 "point estimate" 1 "95% C.I." ))
		...
		if ("`i'" == "acor"){
			if("`ci'" == "on"){
			    graph twoway rarea acor_UCI acor_LCI acor_grid, color(gs14) || line acor_ecdf acor_grid, ytitle("Cumulative Distribution") ///
	                              xtitle("Autocorrelation") title("Empirical CDF Estimation for Autocorrelation of order `acor_order'") legend(cols(2) order(2 "point estimate" 1 "95% C.I." ))
